function [ellipses, posi, L, points] = ellipseDetectionLU(I, Tac, Tr, specified_polarity, verbose)
%input:
% I: input image
...
% vol. 32, no. 4, pp. 722?C732, 2010.
    ...
    [y, x]=find(edge);%??????0????????(y)????(x)??????
    [mylabels,labels, ellipses,pcl] = ellipseDetection(candidates ,[x, y], normals, unit_dis_tolerance, normal_tolerance, Tmin, angleCoverage, I);%?????????? 0.5% 20?? 0.6 180?? 
    if verbose == true
        disp('-----------------------------------------------------------');
