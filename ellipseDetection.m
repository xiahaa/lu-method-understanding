...
%C??   ????????????????????????????????????????????????????????(x,y,a,b,phi)
function varargout = ellipseDetection(candidates, points, normals, distance_tolerance, normal_tolerance, Tmin, angleCoverage, E)
    labels = zeros(size(points, 1), 1);
    mylabels = zeros(size(points, 1), 1);%????
    ellipses = zeros(0, 5);
    pointcloud = cell(size(candidates, 1),1);
...
%    disp('????????????????????????????????????angleLoop??????????????????????angleCoverage??????????????????????????????????????????????????????????????????????????????????????????????????????');
    for angleLoop = 1 : length(angles)
        ...
        [L2, L, C, validCandidates,pcl] = subEllipseDetection( candidates, points(idx, :), normals(idx, :), distance_tolerance, normal_tolerance, Tmin, angles(angleLoop), E, angleLoop);
        ...
        if (size(C, 1) > 0)
            for i = 1 : size(C, 1)
                ...
                if (~flag)%????????????????????????????(circles)??
                    ...
                    pointcloud{size(ellipses, 1)} = pcl{i};
                end
            end
        end
    end
    pointcloud(size(ellipses, 1)+1:end) = [];
%    t2 = clock;
%    disp(['????????????????',num2str(etime(t2,t1))]);
    varargout{1} = mylabels;
    varargout{2} = labels;
    varargout{3} = ellipses;
    varargout{4} = pointcloud;
end
